...
public class Test_006 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for (int i = 0; i < 5; i++)                      //for 문을 통해서 console 창에 모든 Array 베열 안에 있는 0 ~ 4까지
        {                                               //내용을 표시
            Debug.Log(array[i]);
        }
        ...
        for (int i = 0; i < points.Length; i++)         //베열의 길이를 가져오는 변수 (배열이름.Length) => 숫자로 나온다 -> 5
        {
        ...
        int average = sum / points.Length;              //베열의 합 값에서 배열 길이 만큼 나눈다.   (평균값)
